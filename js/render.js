        ...
        <div class="${sidebarShellClass}" ${sidebarShellStyle} id="sidebar-scroll" data-scroll>
            ...
            <div class="flex-1 overflow-y-auto py-4 custom-scrollbar">
                <div class="flex items-center justify-between px-4 group mb-2">
                    <span class="text-xs font-semibold uppercase tracking-wider text-gray-400">${L('sidebar.projects')}</span>
                    <button onclick="window.dispatch('initNewProject')" class="ml-auto inline-flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-xl bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 shadow-sm" title="${L('sidebar.newProject')}">${Icon('plus', '', 14)} ${L('sidebar.newProject')}</button>
                </div>
                <div class="space-y-1"
                     ...
                     ondrop="window.dispatch('projectContainerDrop', event)">${projectsHtml || (state.ui.projectsLoading ? `
                        <div class="px-4 py-6 flex flex-col items-center text-gray-400 text-xs gap-2">
                            ...
                            <span>${L('sidebar.loadingProjects')}</span>
                        </div>
                     ` : '')}</div>
            </div>
            <div class="relative border-t border-gray-200 bg-[#f3f4f6]">
                <div class="h-16 flex items-center px-4">
                    ...
                    <div class="flex flex-col overflow-hidden flex-1">
                        ...
                        <span class="text-xs text-gray-500 opacity-80 truncate">${L('common.online')}</span>
                    </div>
                    <div class="relative">
                        <button onclick="window.dispatch('toggleSettingsPopover')" class="ml-2 p-1 rounded hover:bg-gray-200 text-gray-400 hover:text-gray-700" title="${L('settings.menu')}">
                            ${Icon('settings', '', 16)}
                        </button>
                        ${state.ui.settingsPopoverOpen ? `
                            <div class="absolute bottom-full right-0 mb-2 z-40 bg-white border border-gray-200 rounded-xl shadow-2xl py-1.5 w-48">
                                <button onclick="window.dispatch('openProfileModal')" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                                    ${Icon('user', '', 16)} ${L('settings.profileEntry')}
                                </button>
                                <div class="relative">
                                    <button onclick="window.dispatch('toggleLanguagePicker')" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                                        ${Icon('languages', '', 16)} ${L('settings.language')}
                                    ...
                                    ${state.ui.languagePickerOpen ? `
                                        <div class="absolute left-full bottom-0 ml-1 bg-white border border-gray-200 rounded-xl shadow-2xl py-1.5 w-40">
                                            ...
                                            <button onclick="window.dispatch('setLocale', 'ja')" class="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 flex items-center gap-2 ${state.locale === 'ja' ? 'text-indigo-600 font-semibold' : 'text-gray-700'}">
                                                ${state.locale === 'ja' ? Icon('check', '', 14) : '<span class="w-3.5 inline-block"></span>'} ${L('settings.languageJa')}
                                            </button>
                                        </div>
                                    ` : ''}
                                </div>
                            </div>
                            <div class="fixed inset-0 z-30" onclick="window.dispatch('closeSettingsPopover')"></div>
                        ` : ''}
                    </div>
                </div>
            </div>
        ...
        <div class="relative w-full h-full">
            ...
            ${state.activeView?.type !== 'welcome' ? `
                <button onclick="window.dispatch('goMobileSidebar')"
                    ...
                    ${Icon('chevron-left', '', 16)} ${L('sidebar.backToProjects')}
                </button>
            ` : ''}
        </div>
    `;
            ...
            <div class="flex-1 flex flex-col h-full bg-[#f3f4f6] overflow-y-auto" id="main-scroll" data-scroll>
                <div class="bg-white border-b px-8 py-10 relative">
                    <div class="absolute top-8 right-8 flex items-center gap-2">
                        <button onclick="window.dispatch('openProjectGantt', '${p.id}')"
                            class="flex items-center px-3 py-1.5 rounded-lg border border-indigo-200 text-indigo-600 hover:bg-indigo-50 text-sm font-medium transition-colors">
                            ${Icon('bar-chart-3', 'mr-2', 16)} ${L('project.gantt')}
                        </button>
                        <button onclick="window.dispatch('openConfirmModal', '${isOwner ? 'delete_project' : 'leave_project'}', '${p.id}', '${isOwner ? L('project.delete') : L('project.leave')}', '${isOwner ? '确定要删除这个项目吗？删除后，项目下的所有任务也会被删除，所有成员都将退出。此操作不可撤销。' : '确定要退出这个项目吗？你将不再是该项目的成员。'}')"
                            class="flex items-center px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors
                            ...
                            ${isOwner ? L('project.delete') : L('project.leave')}
                        </button>
                    </div>
        ...
        return `
                        ...
                        <div class="flex items-center space-x-4">
                            ${t.isLocked ? `
                                <div class="flex items-center pr-5">
                                    ...
                                    <div class="flex flex-col items-end justify-center">
                                         <span class="text-[10px] text-gray-400 font-medium leading-none mb-1">${L('task.elapsed')}</span>
                            ...
                            ` : `
                                <div class="px-4 py-2 rounded-full bg-gray-50 text-gray-500 font-medium text-sm border border-gray-200 flex items-center">
                                    ${t.completed ? `<div class=\"w-2 h-2 rounded-full bg-gray-400 mr-2\"></div>${L('task.completed')}` : `<div class=\"w-2 h-2 rounded-full bg-green-500 mr-2\"></div>${L('task.idleEditable')}`}
                                </div>
                            `}
                        </div>
                    `}
                    ...
                    <div class="${isMobile ? 'w-full space-y-5' : 'max-w-4xl mx-auto space-y-8'}">
                        ...
                        <div class="p-4 rounded-2xl border-2 border-dashed transition-all bg-white shadow-sm"
                            ...
                            <div class="flex items-center justify-center gap-3 min-h-[52px]">
                                <button onclick="window.dispatch('openTaskGantt', '${t.id}')"
                                    class="flex items-center px-3 py-2 rounded-lg bg-white border border-indigo-200 text-indigo-600 hover:bg-indigo-50 shadow-sm transition-all text-sm font-medium">
                                    ${Icon('bar-chart-3', 'mr-2', 16)} ${L('task.gantt')}
                                </button>
                                <button onclick="window.dispatch('openEditHistoryModal', '${t.id}')"
                                    ...
                                    ${Icon('file-clock', 'mr-2', 16)} ${L('task.progressHistory')}
                                </button>
                                ${!t.isLocked ? `
                                    <button onclick="window.dispatch('startTask', '${t.id}')"
                                        ...
                                        ${Icon('play', 'mr-2', 18)} ${t.completed ? L('task.completed') : L('task.startWork')}
                                    </button>
                                ` : isMe ? `
                                    <button onclick="window.dispatch('openActionModal', '${t.id}')"
                                        ...
                                        ${Icon('square', 'mr-2', 16)} ${L('task.finishWork')}
                                    </button>
                                ` : `
            ...
            : (githubLink ? L('file.noGithubVersion') : L('file.notUploaded'))}
                            ...
                            <div class="flex gap-2 mb-4">
                                <button onclick="window.dispatch('openTaskGantt', '${t.id}')"
                                    class="flex items-center px-4 py-2 rounded-lg bg-white border border-indigo-200 text-indigo-600 hover:bg-indigo-50 shadow-sm transition-all text-sm font-medium">
                                    ${Icon('bar-chart-3', 'mr-2', 16)} ${L('task.gantt')}
                                </button>
                                <button onclick="window.dispatch('openHistoryModal', '${t.id}')"
                                    ...
                                    ${Icon('history', 'mr-2', 16)} ${L('task.history')}
                                </button>
                                <button onclick="window.dispatch('openEditHistoryModal', '${t.id}')"
                                    class="flex items-center px-4 py-2 rounded-lg bg-white border border-gray-300 text-gray-600 hover:bg-gray-50 hover:text-gray-900 shadow-sm transition-all text-sm font-medium">
                                    ${Icon('file-clock', 'mr-2', 16)} ${L('task.editHistory')}
                                </button>
                                <button onclick="window.dispatch('openGithubLinkModal', '${t.id}')"
                                    ...
                                    ${Icon('github', 'mr-2', 16)} ${githubLink ? L('task.linkedGithub') : L('task.linkGithub')}
                                </button>
                            </div>
                            ${githubLink ? `
